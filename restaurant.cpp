...
public:
    ...
    ~BST() { deleteHelper(root); delete zoneTimeList; }
...
public:
    ...
    SukunaRes(int numZones): numZones(numZones), zones(new Q*[numZones]), resHeap({}) { for (int i = 0; i < numZones; i++) zones[i] = new Q(); }
    ~SukunaRes(){
        for (int i = 0; i < numZones; i++) delete zones[i];
        delete []zones; resHeap.clear();
    }
    ...
    void removeMultipleZones(int num){
        ...
        for (int i = 0; i < num && !pq.empty(); i++){
            ...
            for (int j = 0; j < num && zones[resID]->count; j++) {
                ...
                cout << temp->result << "-" << resID+1 << endl; delete temp;
            } pq.pop();
    ...
    void printLIFO(int num, int idx) const{ // print last num customers of zone idx (ID idx+1)
        ...
        for (int i = 0; i < num && i < zones[idx]->count; i++, temp = temp->left) cout << idx+1 << '-' << temp->result << endl;
    }
