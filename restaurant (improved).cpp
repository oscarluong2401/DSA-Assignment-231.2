...
string caesarEncode(const string& name){
    unordered_map<char, int> charFreq = freqCounter(name); string result = name;
    for (char &i : result) i = i/32 * 32 + 1 + (i%32 -1 + charFreq[i])%26;
    return result;
}
int mod1024(string binStr){
    string result; unsigned int len = binStr.length();
...
HuffNode* balanceNode(HuffNode* root, bool &rotated){
    ...
    if (balanceFactor(root) > 1){
        ...
        root = rotateRight(root); rotated = true;
    } else if (balanceFactor(root) < -1) {
        if (balanceFactor(root->right) > 0) root->right = rotateRight(root->right);
        root = rotateLeft(root); rotated = true;
    }
    root->left = balanceNode(root->left, rotated); root->right = balanceNode(root->right, rotated);
    return root;
}
HuffNode* balanceTree(HuffNode* root){
    for (int i = 0; i < 3; i++){
        bool rotated = false; root = balanceNode(root, rotated);
...
HuffNode* makeHuffTree(const string& input){
    ...
    while (huffQueue.size() > 1){
        ...
        HuffNode *leftT = huffQueue[0], *rightT = huffQueue[1]; int totalWeight = leftT->weight + rightT->weight;
        huffQueue.erase(begin(huffQueue), begin(huffQueue)+2);
        auto* temp = new HuffNode(' ', totalWeight, leftT, rightT);
        temp = balanceTree(temp); huffQueue.push_back(temp);
    }
    return huffQueue[0];
}
class Customer{
...
void enqueue(vector<int> &list, int num){ list.push_back(num); }
int dequeue(vector<int> &list){
    int pos = list[0]; list.erase(begin(list)); return pos;
}
class BST{
public:
    Customer* root;
    vector<int> zoneTimeList;
    BST(Customer* root = nullptr): root(root), zoneTimeList({}) {}
    ~BST() { deleteHelper(root); }
    Customer* minValueNode(Customer *r){ return r->left ? minValueNode(r->left) : r; }
    Customer* addHelper(Customer* r, int num){ // add customer c to tree r
        if (!r) return new Customer(num);
        if (num < r->result) r->left = addHelper(r->left, num); else r->right = addHelper(r->right, num);
        return r;
    }
    void push(int num){ // add to BST + add to queue
        root = addHelper(root, num); enqueue(zoneTimeList, num);
    }
    void pop(){ // remove front of the queue + remove from BST
        root = removeHelper(root, dequeue(zoneTimeList));
    }
    ...
    void deleteHelper(Customer* r){
        if (!r) return ;
        deleteHelper(r->left); deleteHelper(r->right);
        delete r;
    }
};
...
public:
    int numZones;
    BST** zones;
    GojoRes(int numZones): numZones(numZones), zones(new BST*[numZones]) { for (int i = 0; i < numZones; i++) zones[i] = new BST(); }
    ~GojoRes(){
        for (int i = 0; i < numZones; i++) delete zones[i];
        delete []zones;
    }
    void add(int num) const { zones[num % numZones]->push(num); }
    void printCustomersInOrder(Customer* r){ // LNR
        if (!r) return ;
        printCustomersInOrder(r->left); cout << r->result << endl; printCustomersInOrder(r->right);
    }
    ...
    int nCrMod(int n, int r) const{
        ...
        for (int i = 0; i <= n; i++){
            for (int j = 0; j <= r; j++) nCr[i][j] = (j == 0 || j == i ? 1 : (nCr[i-1][j-1] + nCr[i-1][j])) % numZones;
        } return nCr[n][r];
    }
    void pushBSTPreOrder(Customer* root, vector<int> &v){
        if (!root) return ;
        v.push_back(root->result); pushBSTPreOrder(root->left, v); pushBSTPreOrder(root->right, v);
    }
    int numPermutationsBST(vector<int>& arr){
        if (arr.size() <= 2) return 1 % numZones;
        vector<int> leftTree, rightTree; int root = arr[0];
        for (unsigned int i = 1; i < arr.size(); i++) {
            if (arr[i] < root) leftTree.push_back(arr[i]); else rightTree.push_back(arr[i]);
        } // modulo in each step
        return numPermutationsBST(leftTree) * numPermutationsBST(rightTree) % numZones * nCrMod(arr.size() - 1, leftTree.size()) % numZones;
    }
    void removeAllZones(){
        for (int i = 0; i < numZones; i++){
            vector<int> v; pushBSTPreOrder(zones[i]->root, v);
            int numKicks = numPermutationsBST(v);
            for (int j = 0; j < numKicks && zones[i]->zoneTimeList.size(); j++) zones[i]->pop();
        }
    }
};
class Compare{
...
public:
    int numZones;
    vector<vector<int>> zones; // CAUTION: ZONE 0 = ID 1, +1 is just the extra, take the remainder is okay
    vector<tuple<int, int, int>> resHeap; // tuples of <numOfCustomers, lastUpdate[], zoneID>, use make_heap to heapify for this one

    SukunaRes(int numZones): numZones(numZones), resHeap({}) { for (int i = 0; i < numZones; i++) zones.push_back({}); }
    ...
    void add(int num) {
        int idx = findInHeap(num % numZones);
        if (idx < 0) resHeap.emplace_back(1, ++numUpdates, num % numZones);
        ...
        enqueue(zones[num % numZones], num);
    }
    void removeMultipleZones(int num){
        ...
        for (int i = 0; i < num && !pq.empty(); i++){
            ...
            for (int j = 0; j < num && zones[resID].size(); j++) {
                cout << dequeue(zones[resID]) << "-" << resID+1 << endl;
            } pq.pop();
            int idx = findInHeap(resID); // update the number of customers and last update time
            resHeap[idx] = make_tuple(zones[resID].size(), ++numUpdates, resID);
            make_heap(begin(resHeap), end(resHeap), [](const auto &a, const auto &b){ return a > b; });
            int idxFind = findInHeap(resID); // after heapify, found the index of the zone again and delete if zone is empty
            if (get<0>(resHeap[idxFind]) == 0) {
                resHeap[idxFind].swap(resHeap[resHeap.size() - 1]); resHeap.pop_back();
                make_heap(begin(resHeap), end(resHeap), [](const auto &a, const auto &b){ return a > b; });
            }
        }
    }
    void printLIFO(int num, int idx) const{ // print last num customers of zone idx (ID idx+1)
        for (int i = zones[idx].size()-1; i >= 0 && i + num >= zones[idx].size(); i--) cout << idx+1 << "-" << zones[idx][i] << endl;
    }
    void printMinHeapPreOrder(int num, int idx){ // left = 2*idx + 1, right = 2*idx+2
        if ((unsigned int) idx >= resHeap.size()) return ;
        printLIFO(num, get<2>(resHeap[idx])); printMinHeapPreOrder(num, 2*idx+1); printMinHeapPreOrder(num, 2*idx+2);
    }
};
class TotalRes{
public:
    ...
    GojoRes* resG;
    SukunaRes* resS;
    TotalRes(int maxSize): size(maxSize), lastHuffTree(nullptr), resG(new GojoRes(maxSize)), resS(new SukunaRes(maxSize)) {}
    ~TotalRes() { deleteHuffTree(lastHuffTree); delete resG; delete resS; }
    void LAPSE(const string& name){ // register name to a restaurant
        string res, encoded = caesarEncode(name); HuffNode* newHuff = makeHuffTree(encoded);
        if (freqCounter(name).size() < 3 || (isalpha(newHuff->data) && freqCounter(encoded).size() != 1)) { deleteHuffTree(newHuff); return ; }
        deleteHuffTree(lastHuffTree); lastHuffTree = newHuff;
        for (unsigned int i = 0; i < name.length(); i++) res += huffEncode(encoded[i], newHuff);
        int result = res.empty() ? 0 : mod1024(res);
        if (result % 2 == 1) resG->add(result); else resS->add(result);
    }
...
void simulate(const string& filename) {
    ...
    string str, maxsize, name, num; TotalRes* JJK;
    while (ss >> str) {
        if (str == "MAXSIZE") { ss >> maxsize; JJK = new TotalRes(stoi(maxsize)); }
        else if (str == "LAPSE") { ss >> name; JJK->LAPSE(name); }
        else if (str == "KOKUSEN") { JJK->KOKUSEN(); }
        else if (str == "KEITEIKEN") { ss >> num; JJK->KEITEIKEN(stoi(num)); }
        else if (str == "HAND") { JJK->HAND(); }
        else if (str == "LIMITLESS") { ss >> num; JJK->LIMITLESS(stoi(num)); }
        else { ss >> num; JJK->CLEAVE(stoi(num)); }
    } delete JJK;
}